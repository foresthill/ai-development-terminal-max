...
**プロジェクトの切替**：グリッドには一度に1プロジェクトのエージェントだけが表示されます。上部の帯にあるタブが開いているプロジェクトで、クリックか `Alt+P` で巡回します。**1プロジェクトしか無いと `Alt+P` は何も起きません**（切替先が無いため）。**📁 folder** / **⎇ clone** で別プロジェクトを開いてから使ってください。**✦ macro** で全プロジェクトを一覧できます。
